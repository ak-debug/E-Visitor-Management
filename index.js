...
app.post("/visitee/scheduleMeeting", isAuthenticated, (req, res) => {
  ...
  conn.query(sql, [meetingTime, visitorId, visiteeId], (err, result) => {
    if (err) throw err;

    // After successfully scheduling the meeting, send the E-Pass message
    let sqlVisitor = "SELECT * FROM visitors WHERE id = ?";
    conn.query(sqlVisitor, [visitorId], (err, visitorResult) => {
      if (err) {
        console.error(err);
        return res.redirect("/visitee/dashboard");
      }

      let visitor = visitorResult[0];
      let mobile = "+91" + visitor.mobile_no;
      let epassUrl = "www.name.com/epass/" + visitor.id + "/" + visitor.token;
      let MobileBody = "Your Meeting Has Been Scheduled.\nHere is Your E-Pass:\n";
  ...
  //Invalidate the QR
  app.get('/visitor/scanQR/:visitorId/:qrCode', (req, res) => {
    let visitorId = req.params.visitorId;
    let qrCode = req.params.qrCode;
  
    // Assuming the QR code contains the visitor's id
    if (visitorId === qrCode) {
      // Check the status of checkin and checkout
      let checkSql = "SELECT checkin, checkout FROM visitors WHERE id = ?";
      conn.query(checkSql, [visitorId], (err, result) => {
        if (err) {
          console.error(err);
        ...
        if (result.length > 0) {
          let visitor = result[0];
  
          if (!visitor.checkin) {
            // If the visitor hasn't checked in yet, perform checkin
            let sqlCheckin = "UPDATE visitors SET checkin = NOW() WHERE id = ?";
            conn.query(sqlCheckin, [visitorId], (err, result) => {
              if (err) {
                console.error(err);
                return res.send('error');
              }
          ...
          } else if (!visitor.checkout) {
            // If the visitor has checked in but not checked out, perform checkout
            let sqlCheckout = "UPDATE visitors SET checkout = NOW() WHERE id = ?";
            conn.query(sqlCheckout, [visitorId], (err, result) => {
              if (err) {
                console.error(err);
                return res.send('error');
              }
              res.send('checked_out');
            });
          } else {
            // The visitor has already checked out
            res.send('already_checked_out');
          }
        } else {
          // No visitor found with the given ID
          res.send('invalid_visitor');
        }
      });
    } else {
      // Invalid QR code
